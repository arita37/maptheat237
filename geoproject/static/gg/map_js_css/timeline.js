(function ($) {
    ...
    $.fn.navigationWizard = function (options) {
        ...
        var lookup = {};
        debugger
        localStorage.locationData = undefined;
        localStorage.heatmapData = undefined;
        localStorage.heatmapRadius = 0;
        //Load Locations
        ...
        //        if (data.length > 0) {
        if (HeatmapGlobal.icons.length > 0)
            localStorage.locationData = JSON.stringify(HeatmapGlobal.icons);
        //        }
        //    }
        //});
        ...
        debugger
        var steps = '';
        if (HeatmapGlobal.heatmap.Locations.length > 0) {
            localStorage.heatmapData = JSON.stringify(HeatmapGlobal.heatmap.Locations);
            localStorage.heatmapRadius = HeatmapGlobal.heatmap.Radius;

            $.each(HeatmapGlobal.heatmap.Locations, function (index, value) {
                var date = value.date;
                if (!(date in lookup)) {
                    lookup[date] = 1;
                    steps += ['<li>',
                    '<a href="#nav' + (index + 1) + '" data-index="' + (index + 1) + '" class="' + (index != 0 || "active") + '" data-date="' + value.date + '" data-long="' + value.long + '" data-lat="' + value.lat + '">' + value.date + '</a>',
                     '</li>'
                    ].join('');
                }
            });
        ...
        eventWidth = eventStyle.getPropertyValue("width");
            ...
            function setStepPosition(navComponent, min) {
                for (var i = 0; i < navComponent['timelineEvents'].length; i++) {
                    ...
                    navComponent['timelineEvents'].eq(i).css('left', distanceNorm * min - 50 + 'px');
                }
            }
