    ...
    $.widget("ui.tabulator", {
            ...
            if (self.options.pagination) {
                for (var prop in self.lang.pagination) {
                    ...
                    if (propParts.length > 1) {
                        element.attr("title", self.lang.pagination[prop]).attr("aria-label", self.lang.pagination[prop]);
                    }
                    else {
        ...
        redraw: function (fullRedraw) {
            ...
            if (self.element.innerHeight() > 0) {
                ...
                msg.css({
                    "margin-top": (self.element.innerHeight() / 2) - (msg.outerHeight() / 2)
                })
            }
        ...
        _renderGroupHeader: function (group) {
            ...
            var arrow = $("<div class='tabulator-arrow'></div>").on("click", function () {
            ...
            $(".tabulator-group-header", group).html(arrow).append(self.options.groupHeader(group.data("value"), $(".tabulator-row", group).length, data));
        ...
        _showLoader: function (self, msg) {
            if (self.options.showLoader) {
                ...
                $(".tabulator-loader-msg", self.loaderDiv).css({
                    "margin-top": (self.element.innerHeight() / 2) - ($(".tabulator-loader-msg", self.loaderDiv).outerHeight() / 2)
                });
        ...
        _resizeCol: function (index, width) {
            var self = this;
            $(".tabulator-cell[data-index=" + index + "], .tabulator-col[data-index=" + index + "]", this.element).css({
                width: width
            });
        ...
        _vertAlignColHeaders: function () {
            ...
            if (self.header) {
                ...
                $(">.tabulator-col, >.tabulator-col-row-handle", self.header).css({
                    ...
                    "height": self.header.innerHeight() + "px"
                });
                ...
                if (self.options.colVertAlign !== "top") {
                    cols.each(function () {
                        ...
                        col.css({
                            "padding-top": col.innerHeight() - (self.options.colVertAlign === "bottom" ? height : height * 1.5)
                        })
                    });
                }
        ...
        _colLayout: function (forceRedraw) {
            ...
            if (self.options.colResizable) {
                ...
                $(".tabulator-col", self.header).append(handle).append(prevHandle);
                ...
                self.element.on("mousemove", function (e) {
                    if (self.mouseDrag) {
                        self.mouseDragElement.css({
                            width: self.mouseDragWidth + (e.screenX - self.mouseDrag)
                        })
            ...
            $(".tabulator-col[data-sortable=true] .tabulator-col-content", self.header).data("sortdir", "desc").append(arrow.clone());
            ...
            function updateFilter() {
                ...
                $(".tabulator-header-filter", self.header).each(function () {
                    ...
                    if (typeof filterVal !== "undefined" && filterVal !== "") {
                        ...
                        filters[element.closest(".tabulator-col").data("field")] = {
                            value: filterVal
        ...
        _styleRows: function (minimal) {
            ...
            if (self.element.is(":visible")) {
                $(".tabulator-row", self.table).each(function () {
                    $(".tabulator-cell, .tabulator-row-handle", $(this)).css({
        ...
        _resizeRow: function (row) {
            $(".tabulator-cell, .tabulator-row-handle", row).css({
                "height": ""
            });
            $(".tabulator-cell, .tabulator-row-handle", row).css({
        ...
        formatters: {
            ...
            , progress: function (value, data, cell, row, options, formatterParams) { //progress bar
                ...
                var color = formatterParams && formatterParams.color ? formatterParams.color : "#b20000";
            ...
            , progress: function (cell, value, data) {
                ...
                cell.on("mousemove", function (e) {
                    if (bar.data("mouseDrag")) {
                        bar.css({
                            width: bar.data("mouseDragWidth") + (e.screenX - bar.data("mouseDrag"))
                        })
                    }
                });
                ...
                cell.on("keydown", function (e) {
                    ...
                    case 39: //right arrow
                        bar.css({
                            "width": bar.width() + cell.width() / 100
                        });
                        break;
                    case 37: //left arrow
                        bar.css({
